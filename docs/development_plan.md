...
* **주요 작업**:
    ...
    6.  **홀카드 사용 방식 추상화**: `hole_cards.use_constraint` 규칙(`any`, `exact` 등)에 따라 핸드 조합을 생성하는 로직을 인터페이스 기반으로 추상화하고, 룰에 맞는 구현체를 동적으로 사용하도록 변경한다.
        - [x] **핸드 조합 생성 로직 추상화**: 홀카드와 커뮤니티 카드를 조합하여 5장짜리 핸드를 만드는 로직을 `HandIterator`와 같은 인터페이스로 추상화.
            - [x] **카드 조합 헬퍼 함수 구현**: `combinations` 함수를 `pkg/poker/combinations.go`에 구현하고 단위 테스트 및 디버그 로그 추가.
        - [x] **`Any` 방식 구현체**: 기존 NLH, PLS7처럼 홀카드를 자유롭게 사용하는(`0` ~ `N`장) 핸드 조합 생성 로직 구현.
        - [x] **`Exact` 방식 구현체**: PLO, PLO8처럼 홀카드를 지정된 개수만큼 반드시 사용해야 하는(`정확히 N`장) 핸드 조합 생성 로직 구현.
        - [x] **평가 로직(`evaluation.go`) 수정**: `BestHand` 함수가 `HandIterator`를 사용하여 룰에 맞는 최상의 핸드를 찾도록 리팩토링.
        - [x] **기존 룰 업데이트**: NLH, PLS, PLS7 룰 파일(YAML)과 `GameRules` 구조체가 새로운 추상화 방식을 사용하도록 수정.
        - [x] **PLO/PLO8 룰 추가**: PLO, PLO8 게임 룰(YAML) 파일 추가.
        - [x] **PLO8 하이/로 평가 로직 구현**:
            - [x] 최고의 하이 핸드 판정.
            - [x] 8-or-better 조건에 맞는 최고의 로우 핸드 판정.
            - [x] 하이/로 핸드에 따른 팟 분배 로직.
        - [x] **테스트**:
            - [x] 리팩토링된 평가 로직이 기존 게임(NLH, PLS, PLS7)에서 문제없이 동작하는지 회귀 테스트 작성.
            - [x] PLO 핸드 평가에 대한 단위 테스트 작성.
            - [x] PLO8 하이/로 핸드 평가 및 팟 분배에 대한 단위 테스트 작성.
...
### **16단계: 엔진 리팩토링, GoDoc 추가 및 주요 버그 수정**

*   **목표**: `pkg` 디렉토리로 코드를 재배치하여 엔진의 모듈성을 강화하고, GoDoc 문서화를 통해 외부 사용성을 높이며, 테스트 과정에서 발견된 주요 버그를 수정하여 안정성을 확보한다.
*   **주요 작업**:
    *   **엔진 리팩토링**:
        - [x] `internal/game` 패키지를 `pkg/engine`으로 이동하여 외부에서 재사용 가능한 구조로 변경.
        - [x] 패키지 이름을 `game`에서 `engine`으로 변경하여 게임의 상태와 흐름을 관리하는 역할을 명확히 함.
    *   **GoDoc 문서화**:
        - [x] `pkg/poker`와 `pkg/engine` 패키지의 모든 public/private 심볼에 대해 상세한 GoDoc 주석 추가.
    *   **버그 수정 및 로그 복원**:
        - [x] `hasFullHouseDraw` 함수가 전체 카드 풀 대신 5장의 핸드 결과만 참고하여 아웃을 잘못 계산하던 버그 수정.
        - [x] `hasLowHandDraw` 함수가 게임 규칙의 `MaxRank`를 따르지 않고 하드코딩된 값으로 로우 핸드를 판정하던 버그 수정.
        - [x] 문서화 과정에서 삭제되었던 `logrus` 디버그 및 트레이스 로그 복원.
* **완료여부**: ✅

---

### **17단계: 기존 CLI 애플리케이션 강화**

*   **최초 칩, 블라인드 설정 기능 구현**
    *   목표: 사용자가 게임 시작 시 초기 칩과 스몰/빅 블라인드 금액을 직접 설정할 수 있도록 한다.
    *   주요 작업:
        - [x] `cmd/root.go`에 `--chips`, `--sb`, `--bb`와 같은 Cobra CLI 플래그 추가
        - [x] `game.NewGame` 생성자에서 해당 플래그 값을 받아 `Game` 및 `Player` 구조체 초기화 시 반영
*   **게임 상태 저장 및 불러오기 기능 구현**
    *   목표: 현재 게임 상태를 파일로 저장하고, 나중에 이어서 할 수 있는 기능을 추가한다.
    *   주요 작업:
...
### **18단계: poker-engine, poker-cli 분리 (모노레포 방식)**

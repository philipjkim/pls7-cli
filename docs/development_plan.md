...
* **주요 작업**:
    ...
    6.  **홀카드 사용 방식 추상화**: `hole_cards.use_constraint` 규칙(`any`, `exact` 등)에 따라 핸드 조합을 생성하는 로직을 인터페이스 기반으로 추상화하고, 룰에 맞는 구현체를 동적으로 사용하도록 변경한다.
        - [ ] **핸드 조합 생성 로직 추상화**: 홀카드와 커뮤니티 카드를 조합하여 5장짜리 핸드를 만드는 로직을 `HandIterator`와 같은 인터페이스로 추상화.
            - [x] **카드 조합 헬퍼 함수 구현**: `combinations` 함수를 `pkg/poker/combinations.go`에 구현하고 단위 테스트 및 디버그 로그 추가.
        - [ ] **`Any` 방식 구현체**: 기존 NLH, PLS7처럼 홀카드를 자유롭게 사용하는(`0` ~ `N`장) 핸드 조합 생성 로직 구현.
        - [ ] **`Exact` 방식 구현체**: PLO, PLO8처럼 홀카드를 지정된 개수만큼 반드시 사용해야 하는(`정확히 N`장) 핸드 조합 생성 로직 구현.
        - [ ] **평가 로직(`evaluation.go`) 수정**: `BestHand` 함수가 `HandIterator`를 사용하여 룰에 맞는 최상의 핸드를 찾도록 리팩토링.
